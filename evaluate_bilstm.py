...
def evaluate(model, data_iterator, params, mark='Test', verbose=False):
    ...
    for _ in range(params.eval_steps):
        ...
        loss = model.neg_log_likelihood(batch_data, batch_tags)
        _, batch_output = model(batch_data) 
        ...
        #print(np.argmax(batch_output, axis=1)[0])
        #pred_tags.extend([idx2tag.get(idx) for indices in np.argmax(batch_output, axis=1)[0] for idx in indices])
